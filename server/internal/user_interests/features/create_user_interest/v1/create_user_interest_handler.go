...
func (c *CreateUserInterestHandler) Handle(
	...
	if __DEV__ {
		...
		var data []repositories.SaveModel
		...
		// Store to cache
		if err := c.inMemoryRepository.SaveUserInterests(query.Actor, &data); err != nil {
			return err
		}
		return nil
	}
	...
	if err != nil {
		var models []repositories.SaveModel
		...
		for _, content := range query.PageContents {
			...
			models = append(models, repositories.SaveModel{
				ID:          query.Actor,
		...
		// Store to the cache
		if err := c.inMemoryRepository.SaveUserInterests(query.Actor, &models); err != nil {
			return err
		}
	}
...
func (c *CreateUserInterestHandler) FindMostSimilar(queryVector []float32, models []repositories.SaveModel, vectorType string) (string, float32, error) {
